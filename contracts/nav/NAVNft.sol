...
abstract contract NAVNft {
	function nftCalculation(IGovernableFund.NAVNFTUpdate[] calldata nft, address safe, address fund, uint256 navEntryIndex, bool isPastNAVUpdate, IGovernableFund.NAVNFTUpdate[] calldata pastNft) external returns (int256) {
		...
		for(uint i=0;i<nft.length;i++) {
			...
			IGovernableFund.NAVNFTUpdate memory nftVal = nft[i];
	        ...
	        int256 normedRetVal;
	        if (nftVal.nftType == IGovernableFund.NAVNFTType.ERC1155){
	        	normedRetVal = nftFloorPrice * int256(IERC721(nftVal.nftAddress).balanceOf(safe));
	        	nftSum += normedRetVal;
	        } else if (nftVal.nftType == IGovernableFund.NAVNFTType.ERC721){
