...
contract RolesV1 is Modifier {
    ...
    mapping(uint16 => bool) rolesUsed;
    mapping(uint16 => address[]) internal roleTargets;
    uint16[] public seenRoles;
    ...
    function allowTarget(
        ...
        roleTargets[role].push(targetAddress);
    }
        ...
        onlyOwner
    {
        ...
        Arrays.removeItem(roleTargets[role], targetAddress);
    ...
    function assignRoles(
        ...
        for (uint16 i = 0; i < _roles.length; i++) {
            if(rolesUsed[_roles[i]] == false) {
                rolesUsed[_roles[i]] = true;
                seenRoles.push(_roles[i]);
            }
    ...
    function totalSeenRolesLength() external view returns (uint256) {
        return seenRoles.length;
    }

    function getRoleTargets(uint16 roleId) external view returns (address[] memory) {
        address[] memory _filteredRoleTargets = new address[](roleTargets[roleId].length);
        uint16 activeCount = 0;
        for (uint16 i=0; i<roleTargets[roleId].length; i++){
            if(roles[roleId].targets[roleTargets[roleId][i]].clearance != Clearance.None){
                _filteredRoleTargets[activeCount] = roleTargets[roleId][i];
                activeCount++;
            }
        }
        return _filteredRoleTargets;
    }
}
